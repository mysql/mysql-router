...
struct URITestData {
  ...
  URIPath expected_path;
  URIQuery expected_query;
  const char *expected_fragment;
};


// pretty printer for the test data
::std::ostream& operator<<(::std::ostream &os, const URITestData &data) {
  return os << data.uri;
}

class URITestsDD: public URITests,
  public ::testing::WithParamInterface<URITestData> {
};

TEST_P(URITestsDD, ParseURIconstruct) {
  auto test_param = GetParam();
  URI u;

  ASSERT_NO_THROW(u = URI(test_param.uri));

  EXPECT_THAT(u.scheme, StrEq(test_param.expected_scheme));
  EXPECT_THAT(u.host, StrEq(test_param.expected_host));
  EXPECT_EQ(u.port, test_param.expected_port);
  EXPECT_THAT(u.username, StrEq(test_param.expected_username));
  EXPECT_THAT(u.password, StrEq(test_param.expected_password));
  EXPECT_EQ(u.path, test_param.expected_path);
  EXPECT_EQ(u.query, test_param.expected_query);
  EXPECT_THAT(u.fragment, StrEq(test_param.expected_fragment));
}

TEST_P(URITestsDD, ParseURIseturi) {
  auto test_param = GetParam();
  URI u;

  ASSERT_NO_THROW(u = URI("ham://foo:bar@host/path?key=value#frag"));
  ASSERT_NO_THROW(u.set_uri(test_param.uri));
...
class URITestsDDThrowing: public URITests,
  public ::testing::WithParamInterface<URITestFailData> {
};

TEST_P(URITestsDDThrowing, FailsParseURI) {
  // call GetParam outside the ASSERT to actually make it work
  URITestFailData expected = GetParam();

  try {
    auto u = URI(expected.uri);
...
::std::ostream& operator<<(::std::ostream &os, const URItoStringTestData &data) {
  return os << data.scheme << "://" << data.username << ":" << data.password << "@" <<
    data.host << ":" << data.port << "/" << "..." << "?" << "..." << "#" << data.fragment;
}

class URItoStringTestsDD: public URITests,
  public ::testing::WithParamInterface<URItoStringTestData> {
};

TEST_P(URItoStringTestsDD, URItoString) {
  auto test_param = GetParam();
  URI u;

  u.scheme = test_param.scheme;
  u.host = test_param.host;
  u.port = test_param.port;
  u.username = test_param.username;
  u.password = test_param.password;
  u.path = test_param.path;
  u.query = test_param.query;
  u.fragment = test_param.fragment;
  std::stringstream ss;

  ss << u;

  EXPECT_THAT(ss.str(), StrEq(test_param.expected_uri));
